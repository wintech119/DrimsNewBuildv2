...
- **PARTIAL Timezone Fix - Audit Trail Corrected**: Fixed critical timezone bug where `datetime.now()` was storing UTC timestamps instead of Jamaica Standard Time. **Completed fixes**: (1) `app/core/audit.py` now uses `jamaica_now()` for all audit field timestamps (create_dtime, update_dtime, verify_dtime), (2) `DistributionPackage` model defaults now use `jamaica_now` instead of `db.func.current_timestamp()`. **ROOT CAUSE**: System timezone environment variable is NOT set, so Python's `datetime.now()` returns UTC time (not Jamaica time). **REMAINING WORK**: 30+ additional `datetime.now()` usages remain in `packaging.py`, `donations.py`, `donation_intake.py`, `operations_dashboard.py`, `reports.py`, `eligibility.py`, and other modules. **IMPACT**: New audit records now store correct Jamaica timestamps; older records may have UTC timestamps. **RECOMMENDED NEXT STEPS**: (1) Add CI lint check to block new `datetime.now()` regressions, (2) Systematically refactor high-risk modules (packaging, donations, intake) to use `jamaica_now()`, (3) Run data audit script to quantify existing UTC-stamped rows before corrective migration. See architect review notes for detailed remediation plan.
