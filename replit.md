...
- **Accurate Batch Allocation Shortfall Calculation**: Fixed shortfall calculation in batch selection drawer to display accurate inventory availability. Previously, when editing allocations, the system subtracted already-allocated quantities from the requested amount before calculating shortfall, causing confusing displays (e.g., "Maximum available: 5 | Shortfall: 640" when the true shortfall was 645). Changed `batch-allocation.js` to pass the full requested quantity to the API instead of the remaining quantity after subtracting allocations. Shortfall now correctly reflects the gap between total requested and total available inventory.
